...
async def chat(req: Request):
    ...
    # ===================== ORIGINAL PROMPT STYLE (UNCHANGED) =====================
    prompt = f"""You are a helpful assistant with only the knowledge that has been provided. Use the context below to answer the user's question also if helpfull use table to explain briefly do not answer anything outside the provided knowledge base.
...
"""
    ...
    attention_mask = torch.ones_like(inputs["input_ids"]).to(llm.device)
    ...
    def stream():
        ...
        Thread(target=llm.generate, kwargs=gen_kwargs).start()
